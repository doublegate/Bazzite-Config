...
class SecurityValidator:
    ...
    def sanitize_service_name(service: str) -> Optional[str]:
        """Sanitize systemd service names"""
        if not isinstance(service, str):
            return None
        
        # Only allow alphanumeric, dots, hyphens, and .service suffix
        import re
...
class GamingModeController:
    ...
    def enable_gaming_mode(self) -> bool:
        ...
        # Define commands as lists to avoid shell injection
        commands = [
            # CPU performance mode (complex shell operation, keep as string but validate)
            ("shell", "echo performance | sudo tee /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor"),

            # GPU maximum performance
            ("list", ["nvidia-settings", "-a", "[gpu:0]/GPUPowerMizerMode=1"]),
            ("list", ["nvidia-settings", "-a", "[gpu:0]/GPUGraphicsClockOffset[3]=150"]),
            ("list", ["nvidia-settings", "-a", "[gpu:0]/GPUMemoryTransferRateOffset[3]=800"]),

            # Disable compositor
            ("list", ["qdbus", "org.kde.KWin", "/Compositor", "suspend"]),

            # Start GameMode
            ("list", ["systemctl", "--user", "start", "gamemoded"]),

            # Network optimization
            ("list", ["sudo", "/usr/local/bin/ethernet-optimize.sh"]),

            # Clear memory caches (complex shell operation, keep as string but validate)
            ("shell", "sudo sync && sudo sh -c 'echo 3 > /proc/sys/vm/drop_caches'"),

            # Set swappiness
            ("list", ["sudo", "sysctl", "vm.swappiness=1"])
        ]
        ...
        for cmd_type, cmd in commands:
            if cmd_type == "list":
                ...
                # For complex shell operations, validate and use shell=True with caution
    ...
    def disable_gaming_mode(self) -> bool:
        ...
        # Define commands as lists to avoid shell injection
        commands = [
            # CPU balanced mode (complex shell operation, keep as string but validate)
            ("shell", "echo schedutil | sudo tee /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor"),

            # GPU adaptive performance
            ("list", ["nvidia-settings", "-a", "[gpu:0]/GPUPowerMizerMode=0"]),
            ("list", ["nvidia-settings", "-a", "[gpu:0]/GPUGraphicsClockOffset[3]=0"]),
            ("list", ["nvidia-settings", "-a", "[gpu:0]/GPUMemoryTransferRateOffset[3]=0"]),

            # Enable compositor
            ("list", ["qdbus", "org.kde.KWin", "/Compositor", "resume"]),

            # Stop GameMode if not needed
            ("list", ["systemctl", "--user", "stop", "gamemoded"]),

            # Reset swappiness
            ("list", ["sudo", "sysctl", "vm.swappiness=10"])
        ]
...
class GameProfileManager:
    ...
    def apply_profile(self, name: str) -> bool:
        ...
        # Apply CPU governor with input validation
        if profile.get('cpu_governor'):
            # Validate CPU governor input
            valid_governors = ['performance', 'powersave', 'schedutil', 'conservative', 'ondemand']
            cpu_governor = profile['cpu_governor']
            if cpu_governor in valid_governors:
                # Use shell=True for pipe operation but with validated input
                subprocess.run(
                    f"echo {cpu_governor} | sudo tee /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor",
                    shell=True,
                    capture_output=True,
                )

        # Apply GPU settings with input validation
        if profile.get('gpu_overclock'):
            # Validate GPU offset inputs (should be integers within reasonable range)
            try:
                gpu_clock_offset = int(profile.get('gpu_clock_offset', 0))
                gpu_memory_offset = int(profile.get('gpu_memory_offset', 0))
                
                # Clamp to reasonable ranges to prevent hardware damage
                gpu_clock_offset = max(-1000, min(1000, gpu_clock_offset))
                gpu_memory_offset = max(-1000, min(1000, gpu_memory_offset))
                
                subprocess.run([
                    "nvidia-settings", "-a", 
                    f"[gpu:0]/GPUGraphicsClockOffset[3]={gpu_clock_offset}"
                ], capture_output=True)
                
                subprocess.run([
                    "nvidia-settings", "-a", 
                    f"[gpu:0]/GPUMemoryTransferRateOffset[3]={gpu_memory_offset}"
                ], capture_output=True)
            except (ValueError, TypeError):
                print(f"{Colors.WARNING}Invalid GPU offset values in profile{Colors.ENDC}")

        # Toggle compositor
        if profile.get('disable_compositor'):
            subprocess.run(["qdbus", "org.kde.KWin", "/Compositor", "suspend"], capture_output=True)
...
class QuickFixUtilities:
    ...
    def fix_steam_issues():
        ...
        commands = [
            ...
            ["rm", "-rf", os.path.expanduser("~/.cache/mesa_shader_cache")],
            ["rm", "-rf", os.path.expanduser("~/.cache/nvidia")],
            ("shell", "rm -rf ~/.local/share/Steam/steamapps/shadercache/*"),  # Requires glob expansion

            # Reset Steam runtime
            ["rm", "-rf", os.path.expanduser("~/.local/share/Steam/ubuntu12_32/steam-runtime")],

            # Fix Steam library permissions (complex find commands)
            ("shell", "find ~/.local/share/Steam/steamapps -type d -exec chmod 755 {} \\;"),
            ("shell", "find ~/.local/share/Steam/steamapps -type f -exec chmod 644 {} \\;"),
        ]

        for cmd in commands:
            if isinstance(cmd, tuple) and cmd[0] == "shell":
                subprocess.run(cmd[1], shell=True, capture_output=True)
            else:
    ...
    def fix_audio_crackling():
        ...
        commands = [
            ...
            # Clear audio cache
            ["rm", "-rf", os.path.expanduser("~/.cache/pipewire")],
    ...
    def optimize_game_files(game_path: str):
        ...
        try:
            ...
            if not any(allowed in path_str.lower() for allowed in [
                'steamapps', 'games', 'steam', 'epic', 'gog', 'lutris'
            ]):
                print(f"{Colors.WARNING}Path does not appear to be a game directory{Colors.ENDC}")
                return
        except Exception:
            print(f"{Colors.FAIL}Invalid game path{Colors.ENDC}")
            return

        # Defragment if on BTRFS
        subprocess.run([
            "sudo", "btrfs", "filesystem", "defragment", "-r", str(resolved_path)
        ], capture_output=True)

        # Preload game files into cache (complex find command)
        subprocess.run(
            f"find {shlex.quote(str(resolved_path))} -type f -exec cat {{}} > /dev/null \\;",
            shell=True,
            capture_output=True,
        )
...
def main():
    ...
    parser.add_argument(
        '--fix',
        choices=['steam', 'audio', 'gpu', 'caches'],
