...
def clean_kernel_params(current_cmdline: str, new_params: list) -> str:
    ...
    # Parse existing parameters using regex to handle quoted values
    existing_params = re.findall(
        r'(?:[^\s\"]|\"(?:\\.|[^\"])*\")+', current_cmdline.strip()
    )
    param_dict = {}
...
def test_kernel_param_deduplication():
    ...
    current = (
        "quiet splash mitigations=auto mitigations=off mitigations=off "
        "processor.max_cstate=3 processor.max_cstate=3"
    )
